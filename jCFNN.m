...
function CFNN=jCFNN(feat,label,kfold,Hiddens,Maxepochs)
if length(Hiddens)==1
    h1=Hiddens(1); net=cascadeforwardnet(h1); 
elseif length(Hiddens)==2
    h1=Hiddens(1); h2=Hiddens(2);
    net=cascadeforwardnet([h1 h2]);
elseif length(Hiddens)==3
    h1=Hiddens(1); h2=Hiddens(2);
    h3=Hiddens(3); net=cascadeforwardnet([h1 h2 h3]);
end
...
acc=mean(Afold); 
CFNN.fold=Afold; CFNN.acc=acc; CFNN.con=confmat; 
fprintf('\n Classification Accuracy (CFNN): %g %%',acc);
end
