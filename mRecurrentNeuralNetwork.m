...
function RNN = mRecurrentNeuralNetwork(feat,label,opts)
% Parameters
Hidden_size   = [10,10];
Max_epochs    = 50; 
kfold         = 10; 
...
if length(Hidden_size) == 1
	h1  = Hidden_size(1); 
  net = layrecnet(1:2,h1); 
elseif length(Hidden_size) == 2
	h1  = Hidden_size(1); 
  h2  = Hidden_size(2); 
  net = layrecnet(1:2,[h1 h2]);
...
if tf == 1
  fprintf('\n Classification Accuracy (RNN-HO): %g %%',100 * acc);
elseif tf == 2
  fprintf('\n Classification Accuracy (RNN-CV): %g %%',100 * acc);
end
end
